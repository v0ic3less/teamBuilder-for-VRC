...
class SchedulePageAPI extends StatefulWidget {
  const SchedulePageAPI({
    super.key,
    this.width,
    this.height,
    this.teamID,
  ...
  @override
  _SchedulePageAPIState createState() => _SchedulePageAPIState();
}

class _SchedulePageAPIState extends State<SchedulePageAPI> {
  ...
  Future<void> fetchData() async {
    ...
    var inputTeamsUpcomingEvents = await http.get(
      Uri.parse(
          'https://www.robotevents.com/api/v2/teams/171624/events?season=190$startPar'),
      headers: headers,
    );
    final teamsEvents =
    ...
    print('eventsDaya has been set print');
    ...
    if (eventsData != null) {
      ...
      upcomingEvents = eventsData
          ...
          .toList();
      print('eventsWithinSixDays variable is set print');
    ...
    if (eventID != null && div != null) {
      ...
      var inputEventsMatches = await http.get(
        Uri.parse(
            'https://www.robotevents.com/api/v2/events/$eventID/divisions/$div/matches'),
        headers: headers,
      );
      var eventsMatches =
      ...
      // teams/matches API
      var matchesList = eventsMatches['data'] as List;
      var teamsMatches = matchesList
          .where((val) => val['alliances']['teams']
              .any((team) => team['team']['id'] == teamID))
          .toList();
      print('blah');

      setState(() {
        FFAppState().update(() {
          FFAppState().APIResponse1 = filteredEvents;
