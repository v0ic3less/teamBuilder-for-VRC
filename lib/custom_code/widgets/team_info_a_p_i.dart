...
class TeamInfoAPI extends StatefulWidget {
  const TeamInfoAPI({
    super.key,
    this.width,
    this.height,
  ...
  @override
  _TeamInfoAPIState createState() => _TeamInfoAPIState();
}

class _TeamInfoAPIState extends State<TeamInfoAPI> {
  ...
  Future<void> fetchData() async {
          ...
          if (location['location'] != null) {
            ...
            if (lat != null && lon != null) {
              ...
              var timeZoneResponse = await http.get(Uri.parse(
                  'https://corsproxy.io/?https://timeapi.io/api/timezone/coordinate?latitude=$lat&longitude=$lon'));
              ...
              if (timeZoneResponse.statusCode == 200) {
                ...
                if (timeZoneInfo != null &&
                    ...
                    DateTime.parse(location['start'])
                        ...
                        0) {
                  print(
                      'print the end time of the event was after the current local timeasdf');
                  print(
                      DateTime.parse(location['end']).add(Duration(hours: 20)));
                  if (location.isNotEmpty && location is List) {
                    print('list was a list print');
                    ...
                    eventID = location['id'];
                    print('eventID was set print');
                    div = location['divisions'][0]['id'];
                    print('div was set print');
                  } else {
    ...
    if (eventID != null && div != null) {
      ...
      print('seasonsSkills eventsTeams $inputEventsTeams');
      var eventsTeamsOutput =
          jsonDecode(inputEventsTeams.body) as Map<String, dynamic>;
      var eventsTeams = eventsTeamsOutput['data'];

      // seasons/skills API
      /*var inputSeasonsSkills = await http.get(
        Uri.parse('https://www.robotevents.com/api/seasons/190/skills'),
        headers: headers,
      );
      print('seasonsSkills $inputSeasonsSkills');
      print('seasonsSkills api successfully retrieved thing');
      var seasonsSkillsOutput =
          jsonDecode(inputSeasonsSkills.body) as Map<String, dynamic>;
      print('seasonsSkills $seasonsSkillsOutput');
      var seasonsSkillsList = inputSeasonsSkills as List;
      print('seasonsSKills $seasonsSkillsList');
      var seasonsSkills = seasonsSkillsList
          .where((val) => val['team']['id'] == teamID)
          .toList();
      print('seasonsSkills $seasonsSkills'');*/

      //teams/awards API

      var inputTeamsAwards = await http.get(
        Uri.parse(
            'https://www.robotevents.com/api/v2/teams/$teamID/awards?per_page=250$seasonPar'),
        headers: headers,
      );
      var teamsAwardsOutput =
          jsonDecode(inputTeamsAwards.body) as Map<String, dynamic>;
      var teamsAwards = teamsAwardsOutput['data'];
      ...
      setState(() {
        FFAppState().update(() {
          FFAppState().APIResponse11 = filteredEvents;
          FFAppState().APIResponse12 = teamsRankings;
          FFAppState().APIResponse13 = teamsMatches;
          FFAppState().APIResponse14 = eventsRankings;
          FFAppState().APIResponse15 = eventsMatches;
          FFAppState().APIResponse16 = eventsSkills;
          FFAppState().APIResponse17 = upcomingEvents;
          FFAppState().APIResponse18 = teamsSkills;
          FFAppState().APIResponse19 = eventsTeams;
          FFAppState().APIResponse10 = teamsAwards;
          //FFAppState().APIResponse20 = seasonsSkills;
          FFAppState().TimeZoneOffset = timeZoneOffset;
        });
      });
      print('print app states set');
    }
  }
