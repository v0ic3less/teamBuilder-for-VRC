...
class RevealsApiCall extends StatefulWidget {
  const RevealsApiCall(
      {Key? key,
      this.width,
      this.height,
      this.responseVar,
  ...
  @override
  State<RevealsApiCall> createState() => _RevealsApiCallState();
}

class _RevealsApiCallState extends State<RevealsApiCall> {
  ...
  Future<void> fetchData() async {
    ...
    if (method == 'events') {
      apiEndpoint =
          'https://www.robotevents.com/api/v2/events?$seasonPar$endPar';
    }
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        } else if (responseVar == 6) {
          FFAppState().update(() {
            FFAppState().APIResponse6 = jsonDecode(response.body);
          });
        }
      });
      FFAppState().update(() {});
